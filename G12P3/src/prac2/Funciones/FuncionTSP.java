...
public class FuncionTSP extends Funcion{
	...
	public void calcular(Cromosoma cromosoma) {
		int dist=0;
		int current,next;
		for(int i=0; i<(cromosoma.getCromosoma().size()-1); i++) {
			current=(int) cromosoma.getCromosoma().get(i).getFenotipo();
			next=(int) cromosoma.getCromosoma().get(i+1).getFenotipo();
			
			dist+=getDist(current,next);
		}
		current=(int) cromosoma.getCromosoma().get(cromosoma.getCromosoma().size()-1).getFenotipo();
		next=(int) cromosoma.getCromosoma().get(0).getFenotipo();
		dist+=getDist(current,next);
		cromosoma.setValor(dist);
		cromosoma.setFitness(23000-dist);
	}
	
	public int getDist(int posUno, int posDos) {
		if(posUno<posDos) {
			return _DIST[posDos][posUno];
		}else {
			return _DIST[posUno][posDos];
		}
	}
